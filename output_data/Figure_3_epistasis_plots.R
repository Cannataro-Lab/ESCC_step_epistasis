...
plot_epistasis_results_with_ci = function(ep_results, current_gene, upper_limit) {
  ...
  # Filter for specific genes to plot
  data_for_compare_selected_genes <- data_for_compare # %>%
  #   filter(other_gene == "TP53" | other_gene == "NOTCH2" | other_gene == "ERBB4" | other_gene == "SETD2" | other_gene == "PPM1D" | other_gene == "NOTCH3")
  #data_for_compare_selected_genes$other_gene <- factor(data_for_compare_selected_genes$other_gene, levels = unique(data_for_compare_selected_genes$other_gene))
  ...
  GOI_plot <- ggplot(data = data_for_compare_selected_genes) +
    ...
    scale_x_discrete(limits = levels(data_for_compare_selected_genes$other_gene)) +
    theme_classic() +
    ...
    geom_text(aes(x=as.numeric(other_gene), y=upper_limit, label = significance), size = geom_text_size) +
  ...
  OTHER_plot <- ggplot(data = data_for_compare_selected_genes) +
    ...
    geom_text(aes(x=as.numeric(other_gene), y=3000, label = significance), size = geom_text_size) +
    scale_color_discrete(name="p-value", breaks=c("yes", "no"), labels=c("p < 0.05", "p ≥ 0.05")) 
    #guides(color = guide_legend(override.aes = aes(label = "|")))
...
notch1_ep_plot <- plot_epistasis_results_with_ci(gene_ep_results, "NOTCH1", 2000)
tp53_ep_plot <- plot_epistasis_results_with_ci(gene_ep_results, "TP53", 6200)
